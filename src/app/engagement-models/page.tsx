...
import { localizeEngagementModel } from "@/i18n/engagement-ar";

export async function generateMetadata() { const ar = (await getLocale()) === "ar"; return buildMetadata({ title: ar ? "نماذج التعاون مع باراكود" : "Engagement Models — Flexible ways to work with Barakode", description: ar ? "طرق واضحة للتعاون في بناء المنتجات والأتمتة بمساعدة الذكاء الاصطناعي والدعم المستمر، مع عروض أسعار حسب نطاق العمل." : "Structured engagement models for product builds, AI automation, and ongoing support. Custom quotes after discovery — no misleading fixed-price tables.", path: "/engagement-models" }); }
...
function ModelCopy({ model, index, dark = false, ar = false }: { model: (typeof ENGAGEMENT_MODELS)[number]; index: number; dark?: boolean; ar?: boolean }) {
  return (
    <Reveal kind={index % 2 ? "right" : "left"} className="engagement-copy">
      ...
      <p className={`mt-4 max-w-xl text-lead ${dark ? "text-ontext-2" : "text-text-2"}`}><span className={dark ? "text-white" : "text-text"}>{ar ? "يناسب: " : "Best for: "}</span>{model.bestFor}</p>
      <div className={`mt-6 border-t pt-5 ${dark ? "border-rule-dark" : "border-rule"}`}>
        <p className={`font-mono text-xs tracking-[.14em] uppercase ${dark ? "text-ontext-3" : "text-text-4"}`}>{ar ? "ما يشمله التعاون" : "What is included"}</p>
...
export default async function EngagementModelsPage() {
  ...
  const models = ar ? ENGAGEMENT_MODELS.map(localizeEngagementModel) : ENGAGEMENT_MODELS;
  return (
    <>
          ...
          <section key={model.slug} id={model.slug} className="engagement-model engagement-editorial flex scroll-mt-20 items-center overflow-hidden bg-paper-sunken py-20">
            <div className="shell w-full"><div className="grid items-center gap-10 lg:grid-cols-[.88fr_1.12fr] lg:gap-20">
              ...
              <Reveal kind="right" className="relative grid grid-cols-[.72fr_1fr] items-end gap-3 sm:gap-4">
                <div className="relative mb-6 h-[15rem] overflow-hidden rounded-t-full shadow-e3 sm:mb-10 sm:h-[22rem]"><Image src={SECONDARY_IMAGES[i]} alt={ar ? "جلسة استكشاف وتخطيط للمشروع" : "Discovery workshop in progress"} fill sizes="(max-width: 640px) 42vw, 24vw" className="object-cover" /></div>
                <div className="relative h-[22rem] overflow-hidden rounded-[1.25rem] shadow-e4 sm:h-[32rem] sm:rounded-[1.5rem]"><Image src={image.src} alt={ar ? "خطة المنتج ومسار التنفيذ" : image.alt} fill sizes="(max-width: 640px) 58vw, 40vw" className="object-cover" /><div className="absolute inset-x-3 bottom-3 rounded-xl bg-white/90 p-3 backdrop-blur sm:inset-x-5 sm:bottom-5 sm:p-4"><span className="font-mono text-[.625rem] text-accent-ink uppercase">{ar ? "تنفيذ مركز" : "Focused delivery"}</span><p className="mt-1 text-xs font-medium text-text sm:text-sm">{model.timeline}</p></div></div>
              </Reveal>
            </div></div>
          </section>
        );
              ...
              <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)] lg:gap-16 xl:gap-24">
                ...
                <Reveal kind="right">
                  <div className="group/product relative aspect-[16/11] overflow-hidden rounded-[var(--radius-lg)] bg-paper-sunken shadow-e4">
                    <Image
                      src={image.src}
                      alt={ar ? "منتج رقمي مخصص على أجهزة مختلفة" : image.alt}
                      fill
          ...
          <section key={model.slug} id={model.slug} className="engagement-model engagement-support relative flex scroll-mt-20 items-center overflow-hidden border-t border-rule py-20">
            <div className="shell relative w-full"><div className="grid items-center gap-10 lg:grid-cols-[1fr_.82fr] lg:gap-20">
              ...
              <Reveal kind="right" className="rounded-[1.5rem] border border-rule bg-white p-4 shadow-e3 sm:rounded-[1.75rem] sm:p-6">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-rule pb-5"><span className="font-mono text-xs text-text-3 uppercase">{ar ? "حالة المنتج" : "Product health"}</span><span className="flex items-center gap-2 text-xs text-emerald-700"><span className="size-2 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,.35)]" />{ar ? "دعم مستمر" : "Active support"}</span></div>
                <div className="mt-5 grid grid-cols-2 gap-3">{(ar ? ["المراقبة", "التحديثات", "الأداء", "الأمان"] : ["Monitoring", "Updates", "Performance", "Security"]).map((item, n) => <div key={item} className="rounded-xl border border-rule bg-paper-sunken p-4"><span className="font-mono text-[.625rem] text-accent-ink">0{n + 1}</span><p className="mt-5 text-sm font-medium text-text">{item}</p><div className="mt-3 h-1 overflow-hidden rounded-full bg-rule"><div className="h-full rounded-full bg-accent" style={{ width: `${72 + n * 7}%` }} /></div></div>)}</div>
                <div className="relative mt-4 h-32 overflow-hidden rounded-xl"><Image src={SECONDARY_IMAGES[i]} alt={ar ? "اختبار جودة المنتج وتسليمه" : "Quality assurance and product delivery"} fill sizes="36vw" className="object-cover" /><div className="absolute inset-0 bg-gradient-to-r from-black/70 to-transparent" /><p className="absolute bottom-4 left-4 text-sm font-medium text-white">{model.timeline}</p></div>
              </Reveal>
            </div></div>
          </section>
        );
      })}
      <JsonLd data={breadcrumbSchema([{ name: ar ? "الرئيسية" : "Home", path: "/" }, { name: ar ? "نماذج التعاون" : "Engagement Models", path: "/engagement-models" }])} />
    </>
  );
}
